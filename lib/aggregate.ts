...
export function getRankedCountries(
  ...
  // Determine indicator direction for tiebreaking by rawValue
  ...
  const withData = scored.filter((e) => e.score !== -1).sort((a, b) => {
    ...
    // Same rounded score: sort by rawValue using indicator direction
    if (a.rawValue !== undefined && b.rawValue !== undefined) {
      if (indicatorDirection === 'lower_is_better') {
        return a.rawValue - b.rawValue; // lower raw = better = higher rank
      }
      return b.rawValue - a.rawValue; // higher raw = better = higher rank
    }
    return 0;
  });
